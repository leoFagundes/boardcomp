...
import { teamLabel, teamColor, statusLabel, formatDate } from "@/lib/utils/helpers";
import { Trophy, Gamepad2, Users, TrendingUp, ArrowRight, Clock } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const waitingMatches = matches.filter((m) => m.status === "waiting").slice(0, 3);
  const activeMatches = matches.filter((m) => m.status === "active").slice(0, 3);
  ...
  const stats = [
    ...
    { label: "Partidas", value: myMatches.length, icon: Gamepad2, color: "#3B82F6" },
    { label: "Jogos Disponíveis", value: games.length, icon: Users, color: "#8B5CF6" },
  ];
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${tc}15 0%, #0A0A0A 70%)`, border: `1px solid ${tc}30` }}
      >
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href="/jogos" className="text-amber-400 text-sm hover:text-amber-300 flex items-center gap-1">
              Ver todos <ArrowRight size={14} />
            </Link>
          </div>
          {waitingMatches.length === 0 ? (
            <p className="text-coal-500 text-sm text-center py-6">Nenhuma partida aguardando</p>
          ) : (
            <div className="space-y-3">
              {waitingMatches.map((m) => (
                <Link key={m.id} href={`/jogos/${m.id}`} className="flex items-center justify-between p-3 rounded-lg bg-coal-800 hover:bg-coal-700 transition-all border border-coal-700 hover:border-coal-600">
                  <div>
                    <div className="font-medium text-coal-100 text-sm">{m.gameName}</div>
                    <div className="text-coal-400 text-xs mt-0.5">{m.players.length} jogadores inscritos</div>
                  </div>
        ...
        <div className="card p-6">
          ...
          {activeMatches.length === 0 ? (
            <p className="text-coal-500 text-sm text-center py-6">Nenhuma partida em andamento</p>
          ) : (
            <div className="space-y-3">
              {activeMatches.map((m) => (
                <Link key={m.id} href={`/jogos/${m.id}`} className="flex items-center justify-between p-3 rounded-lg bg-coal-800 hover:bg-coal-700 transition-all border border-coal-700 hover:border-coal-600">
                  <div>
                    <div className="font-medium text-coal-100 text-sm">{m.gameName}</div>
                    <div className="text-coal-400 text-xs mt-0.5">{m.players.length} jogadores</div>
                  </div>
      ...
      {myMatches.length > 0 && (
        <div className="card p-6 mt-6">
          <h2 className="font-bold text-coal-100 mb-4">Minhas Partidas Recentes</h2>
              ...
              return (
                <Link key={m.id} href={`/jogos/${m.id}`} className="flex items-center justify-between p-3 rounded-lg hover:bg-coal-800 transition-all">
                  <div className="flex items-center gap-3">
                    <span className="text-xl">{won ? "🏆" : m.status === "active" ? "⚔️" : "🎲"}</span>
                    <div>
                      <div className="font-medium text-coal-100 text-sm">{m.gameName}</div>
                  ...
                  <div className="flex items-center gap-2">
                    {m.status === "finished" && won && (
                      <span className="text-amber-400 text-xs font-bold">+1 pt</span>
                    )}
